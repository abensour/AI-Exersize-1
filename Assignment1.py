...
if __name__=='__main__':
    ...
    print('----------- Question 2 ----------- ')
    best_model_logistic_q2, evaluation_LR_q2 = reproduce_LR(x, y)
    best_model_rf_q2, evaluation_rf_q2 = reproduce_RF(x, y)
    best_model_xgboost_q2, evaluation_xg_q2 = reproduce_xgboost(x, y)
    print(f'Evaluation:\n{pd.concat([evaluation_LR_q2, evaluation_rf_q2, evaluation_xg_q2]).to_string()}')

    print('----------- Question 3 ----------- ')
    new_x = add_features(x)
    print(f'The new data set is\n{new_x}')
    best_model_logistic_q3, evaluation_LR_q3 = reproduce_LR(new_x, y)
    best_model_rf_q3, evaluation_rf_q3 = reproduce_RF(new_x, y)
    best_model_xgboost_q3, evaluation_xg_q3 = reproduce_xgboost(new_x, y)
    print(f'Evaluation:\n{pd.concat([evaluation_LR_q3, evaluation_rf_q3, evaluation_xg_q3]).to_string()}')

    print('----------- Question 4 ----------- ')
    best_model_lgbm, evaluation_lgbm = evaluate_LGBM(x, y)
    best_model_cat, evaluation_cat = evaluate_CatBoost(x, y)
    print(f'Evaluation:\n{pd.concat([evaluation_lgbm, evaluation_cat, evaluation_xg_q2]).to_string()}')

    print('----------- Question 6 ----------- ')
    explain_using_shap(x, y, best_model_rf_q2, best_model_xgboost_q2, best_model_cat, best_model_lgbm)
